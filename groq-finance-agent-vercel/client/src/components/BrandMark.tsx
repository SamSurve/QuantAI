...
export function BrandMark({ compact = false, className }: { compact?: boolean; className?: string }) {
  return (
    <div className={cn("flex items-center gap-2.5", className)} aria-label="Groq Finance">
      ...
      {!compact && (
        <span className="relative leading-none before:absolute before:-left-2 before:top-1 before:h-7 before:w-px before:bg-[#0e8f83]">
          <span className="block text-[10px] font-bold uppercase tracking-[0.28em] text-[#1e2928]">Groq</span>
          <span className="block font-serif text-[1.42rem] tracking-[-0.055em] text-[#1e2928]">Finance</span>
        </span>
      )}
    </div>
  );
